...
haarcascade_url = "https://raw.githubusercontent.com/kipr/opencv/blob/master/data/haarcascades/haarcascade_frontalface_default.xml"

# This function is meant to give the skin color of the person by detecting face and then
# applying k-Means Clustering.
def get_skin_color(img):
	if not os.path.exists(haarcascade_path):
		response = requests.get(haarcascade_url, stream=True)
		if response.status_code == 200:
			with open(haarcascade_path, 'wb') as file:
				for chunk in response.iter_content(chunk_size=8192):
					file.write(chunk)
			print(f"File downloaded successfully: {haarcascade_path}")
		else:
			print(f"Failed to download file. Status code: {response.status_code}")
	else:
		print(f"File already exists: {haarcascade_path}")
